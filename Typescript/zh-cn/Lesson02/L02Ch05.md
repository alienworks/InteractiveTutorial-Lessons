...
正如我们在第1课中所描述的，NEO网络上的智能合约本质上是一组能够被其他合约或者用户所调用的方法。利用循环和Switch语句，我们将得以在合约中实现更复杂的组织结构。

### 循环

循环语句允许我们多次执行一个语句或一组语句。TypeScript提供了不同类型的循环来处理循环需求，这些内容在NEO智能合约中同样也可以使用。
...
`break`语句和`continue`语句也可用于控制循环的执行。使用break会完全结束一个循环。而continue语句是跳过当次循环中剩下的语句，直接返回起始处执行下一次循环。其他关键字，`for…of`,`forEach`,`every`和`some`则可以使我们的脚本更具可读性和敏感性。

### Switch语句

当要解析一个或多个条件和执行一个或多个语句时，有分支的决策结构总是很有帮助的。NEO智能合约同时支持`if...else`语句和`switch`语句。举个例子：
...
```
显然，`if...else`语句可以处理是非问题，并相应地执行两个不同的代码块。再加上关键字`else if`，我们的代码就可以连续做出多个判断。

不过，为了解决这样的多选项问题，`switch`语句也许是更好的选择。

## 指令: 

- 添加一个名为`mutate`的新方法，以随机更改Alien的XNA。它以Alien的id和一个`attribute`数字为参数。
- 在方法`mutate`中，将当前区块高度传入方法`randomNumber`中，并使用一个临时变量`randomDigit`存储结果的最后一个数字。
- 实现一个`switch`结构，该结构可以随机更改XNA的特定部分，
    - 当`attribute`为0时，更改Alien的生命形态（数字1-2）;
    - 当`attribute`为1时，更改Alien的力量（数字3-4）;
    - 当`attribute`为2时，更改Alien的速度（数字5-6）;
    - 当`attribute`为3时，更改Alien的重量（数字7-8）;
    - 如果都不是，不执行任何语句。
- 用XNA新值更新映射中存储的原值。
